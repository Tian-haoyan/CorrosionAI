...
def clean_default_output_dir(output_dir: Path) -> None:
    """Remove files/folders generated by previous default Colab runs."""
    generated_files = [
        "predictions.csv",
        "sample_ci_summary.csv",
        "sample_ci_summary.txt",
    ]
    generated_dirs = [
        "group_summary",
        "overall_summary",
...
def write_ci_summary_csv(rows: Iterable[Dict[str, object]], save_path: Path) -> None:
    fieldnames = [
        "sample",
        "total_images",
        "predicted_CI_star",
...
def make_ci_summary_row(sample_name: str, pred_count: Dict[str, int]) -> Dict[str, object]:
    total = int(sum(pred_count.values()))

    def pct(cls: str) -> str:
        return f"{safe_divide(pred_count[cls] * 100.0, total):.6f}"
    ...
    return {
        ...
        "prob_R": "",
        "total_images": total,
        "n_R": pred_count["Unweathered-R"],
        "n_A": pred_count["Unweathered-A"],
        "n_C": pred_count["Corroded"],
        "n_E": pred_count["Etched"],
        "n_S": pred_count["Skeletal"],
        "pct_R": pct("Unweathered-R"),
        "pct_A": pct("Unweathered-A"),
        "pct_C": pct("Corroded"),
        "pct_E": pct("Etched"),
        "pct_S": pct("Skeletal"),
        "predicted_CI_star": f"{calculate_ci_star(pred_count):.6f}",
    }


def write_ci_summary_txt(rows: Iterable[Dict[str, object]], save_path: Path) -> None:
    rows = list(rows)
    with save_path.open("w", encoding="utf-8") as f:
        f.write("CorrosionAI prediction summary\n")
        f.write("=" * 60 + "\n")
        f.write("Formula: CI* = (Skeletal + 0.75 * Etched + 0.5 * Corroded) / N * 100\n\n")
        for row in rows:
            f.write(f"Sample: {row['sample']}\n")
            f.write(f"Total images: {row['total_images']}\n")
            f.write(f"Predicted CI*: {row['predicted_CI_star']}\n")
            f.write(
                "Predicted counts: "
                f"R={row['n_R']}, A={row['n_A']}, C={row['n_C']}, "
                f"E={row['n_E']}, S={row['n_S']}\n"
            )
            f.write(
                "Predicted percentages: "
                f"R={row['pct_R']}%, A={row['pct_A']}%, C={row['pct_C']}%, "
                f"E={row['pct_E']}%, S={row['pct_S']}%\n\n"
            )
...
def main() -> None:
    ...
    if not args.evaluate_ground_truth:
        clean_default_output_dir(output_dir)
    ...
    all_rows.append(make_ci_summary_row("OVERALL", overall_pred_count))
